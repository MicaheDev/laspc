...
const PointCloudViewer = ({ lasFileUrl }) => {
  ...
  useEffect(() => {
    ...
    async function fileRender() {
      ...
      let samplingRate = 0.2; // Tomar el 10% de los puntos, puedes ajustar este valor según tus necesidades.
      ...
      const camera = new PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.03, 10);
      ...
      for (let i = 0; i < positions.length; i += 3) {
        ...
        let color = new Color().setHSL(colorValue, 0.3, 0.5);
